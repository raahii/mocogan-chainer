...
def main():
    ...
    ch = 3
    T = 16  # num frames
    Dc = 50 # the dimension of the content vector
    Dm = 10 # the dimension of the motion vector
    D = Dc + Dm
    ...
    # Set up dataset
    train_dataset = MugDataset(args.dataset, T)
    ...
    # Set up models
    gru = GRU(T=T, n_zc = Dc, n_zm=Dm)
    gen = Generator(ch, n_hidden=D)
    image_dis = ImageDiscriminator(ch, use_noise=True, noise_sigma=0.2)
    video_dis = VideoDiscriminator(ch, use_noise=True, noise_sigma=0.1)
    ...
    # trainer.extend(extensions.snapshot_object(
    #     video_dis, 'video_dis_iter_{.updater.epoch}.npz'), trigger=snapshot_interval)
    trainer.extend(extensions.LogReport(trigger=display_interval))
    trainer.extend(extensions.PrintReport([
        'epoch', 'iteration', 'gen/loss', 'image_dis/loss', 'video_dis/loss'
    ]), trigger=display_interval)
    trainer.extend(
        save_video_samples(gru, gen, generate_num, size, ch, T, seed, save_path, ext),
